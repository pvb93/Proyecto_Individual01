...
def recomendacion(titulo, movies, cosine_sim, indices,rating):
    ...
    sim_scores = list(enumerate(cosine_sim[idx]))#list of similarities with index of movies
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)#sort list from highest to lowest similarity
    sim_scores = sim_scores[1:12]#choose first 11 values, the given title is skiped
    movie_indices = [i[0] for i in sim_scores]#extract movie's index from list
...
#ratg90 = st.radio('¿Desea una recomendación basada en la puntuación?',('Yes','No'))

#ml_90 = recomendacion(titulo_elegido90, movies90, cosine_sim90, indices90,ratg90)
...
ratg91 = st.radio('¿Desea una recomendación basada en la puntuación?',('Yes','No'))

ml_91 = recomendacion(titulo_elegido91, movies91, cosine_sim91, indices91,ratg91)
